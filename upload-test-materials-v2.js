 ...
 * Using fetch API with proper FormData handling
 */
...
// Admin token - replace with valid token from your system
...
async function uploadMaterial(category, level = 'A1', tier = 'FREE') {
  try {
    ...
    form.append('title', `${category.title} - Level ${level}`);
    ...
    form.append('tags', JSON.stringify([category.name.toLowerCase(), level]));
